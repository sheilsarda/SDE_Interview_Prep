...
- When both the string and the pattern are empty. Always match. dp[0][0] = true;
- When only the string is empty. Only if the subpattern only consists of *, we have a match.
- When only the pattern is empty. Always not match.
...
- `*` This is slightly hard to deal with. When we encounter a * in the pattern, and assuming that we're currently trying to figure out what `dp[i][j]` is. Then we need to consider two cases if `p[j - 1] == '*'`.
````
Is dp[i - 1][j] true?
